...
| `web/src/pages/`    | Компоненты страниц: Dashboard, EventTypeCreate, Booking, ConfirmedPage.         |
...
- При старте сервера (`initDb()`) таблицы создаются через `CREATE TABLE IF NOT EXISTS` — отдельная миграция не нужна.
- Drizzle-схема в `server/src/db/schema.ts` — **источник типов** для запросов, но **не для создания таблиц** (создание — raw SQL в `index.ts`).
