...
| `server/data.db*` | SQLite БД (игнорируется git) |
...
| `docs/` | `mvp-architecture.md` (схема БД, алгоритм слотов), `issues.md` (список задач) |
...
|---|---|
| TypeSpec | `make spec` |
| Типы сервера | `cd server && npm run typecheck` |
...
| `/book/:username/:slug` | BookingPage | Название ивента |
| `/book/:username/:slug/confirmed/:bookingId` | ConfirmedPage | "You're booked!" |
...
- **Путь по умолчанию:** `server/data.db` (CWD = `server/` при запуске). Тесты используют `:memory:` через `DB_PATH` env. WAL только для файловых БД.
...
- Дефолтный юзер: `alex` / `Alex Sokolov` / `Europe/Moscow` (`INSERT OR IGNORE` в `seed.ts`)
- WAL → файлы `server/data.db-wal`, `server/data.db-shm` — чистить вместе с `server/data.db`
...
| `make types` | `spec` + `openapi-typescript` → `web/src/types/api.ts` |
| `make mock` | Prism mock :4010 |
| `make dev` | Vite :5173 |
| `make dev-backend` | Express :3000 (`tsx watch`) |
| `make build` | `types` + сборка server + web |
| `make test` | vitest run (в `server/`) |
| `make test-watch` | vitest watch (в `server/`) |
| `make db-reset` | `rm -f server/data.db*` + `drizzle-kit push` |
